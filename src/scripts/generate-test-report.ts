...
function main() {
  ...
  const lines: string[] = []
  lines.push('# Mote POS — Test Report')
  ...
  for (const t of securityTests) lines.push(`- ${tick(t.status)} ${t.title}`)
  lines.push('')
  lines.push('## Login Code Generator')
  for (const t of generatorTests) lines.push(`- ${tick(t.status)} ${t.title}`)
  lines.push('')
  lines.push('## Public Pages (Mobile / Tablet / Desktop)')
  for (const t of pageTests) lines.push(`- ${tick(t.status)} ${t.title}`)
  lines.push('')
  ...
  console.log('')
  console.log(bar)
  console.log('  Mote POS — Test Audit Summary')
  console.log(bar)
  console.log(`  Total Tests:     ${total}`)
  console.log(`  Passed:          ${passed} ✅`)
  console.log(`  Failed:          ${failed} ${failed ? '❌' : '✅'}`)
  console.log('')
  console.log('  Security (Workspace Login Code):')
  for (const t of securityTests) console.log(`    ${tick(t.status)} ${t.title.split(' › ').pop()}`)
  console.log('')
  console.log('  Generator:')
  for (const t of generatorTests) console.log(`    ${tick(t.status)} ${t.title.split(' › ').pop()}`)
  console.log('')
